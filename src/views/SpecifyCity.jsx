...
export const SpecifyCity = ({ open, cities, updateCity2, updateState, updateGeocode, openNameTripModal, onClose }) => {
    if (!open) return null

    const handleSelect = (cityName, state, lat, lon) => {
        let geocode = [lat, lon]
        ...
        <>
            ...
            <div className="modal">
                <div className="specifyCity-box">
                    ...
                    <div className="">
                        {cities.map((city, i) => {
                            return <Link onClick={() => handleSelect(city.name, city.state, city.latitude, city.longitude)} key={i}><p className="large">{city.name}, {city.state ? city.state+", " : null} {city.country}</p></Link>
                        })}
